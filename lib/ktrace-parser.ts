...
export interface ParseResult {
  ...
  fileName: string;
  /**
   * The DebugLog Trace Mask found at the beginning of the KTRACE
   * (e.g. "0xFFFFFFFF"). Null if no trace mask line was found.
   */
  traceMask: string | null;
  /** Number of SPEECH_STARTED audio events found in the log. */
  speechStartedCount: number;
  /** Number of SPEECH_STOPPED audio events found in the log. */
  speechStoppedCount: number;
  /**
   * True when there is at least one SPEECH_STARTED event without a
   * matching SPEECH_STOPPED event (i.e. SPEECH_STOPPED events are missing).
   */
  missingSpeechStopped: boolean;
}

/** The expected/complete DebugLog Trace Mask value. */
export const FULL_TRACE_MASK = '0xFFFFFFFF';
...
export function parseKtraceLog(content: string, fileName: string): ParseResult {
  ...
  // Trace mask, e.g. "19:46:53.109 - DebugLog Trace Mask: 0xFFFFFFFF"
  const traceMaskRegex = /DebugLog Trace Mask:\s*(0x[0-9A-Fa-f]+)/;
  // Audio state events, e.g. "OnReActiveAudioEvent( SPEECH_STARTED )"
  const speechStartedRegex = /SPEECH_STARTED/;
  const speechStoppedRegex = /SPEECH_STOPPED/;

  // ---- Trace mask + speech-event detection ----
  let traceMask: string | null = null;
  ...
  for (let i = 0; i < (lines?.length ?? 0); i++) {
    ...
    // Capture the FIRST trace mask (at the beginning of the KTRACE).
    if (traceMask === null) {
      const tmMatch = trimmed.match(traceMaskRegex);
      if (tmMatch) {
        traceMask = (tmMatch[1] ?? '').toUpperCase().replace(/^0X/, '0x');
      }
    }

    // Count audio speech state events.
    if (speechStartedRegex.test(trimmed)) speechStartedCount++;
    if (speechStoppedRegex.test(trimmed)) speechStoppedCount++;
  ...
  return {
    ...
    speechStartedCount,
    speechStoppedCount,
    // Missing SPEECH_STOPPED: there is at least one SPEECH_STARTED that is
    // not matched by a SPEECH_STOPPED event.
    missingSpeechStopped: speechStartedCount > speechStoppedCount,
  };
}
